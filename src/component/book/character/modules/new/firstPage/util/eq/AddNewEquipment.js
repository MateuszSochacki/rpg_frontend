...
export default function AddNewEquipment(props) {
    ...
    const handleAddToCart = (name, category, price,weight) => {
        ...
        if (price.includes("p")) {
            ...
            if (parseInt(p) <= copper) {
                ...
                setCopper(p);
                bas.push(...basket, {name: name, price: price, type: category,weight:weight});
                setBasket(bas);
    ...
    const saveEq = () => {
        ...
        basket.forEach((item) => eqTable.push({name: item.name, type: item.type,weight:item.weight}));
        char = {
    ...
    return (
        ...
        >
                                            ...
                                            if (eq.type === cat) {
                                                return (
                                                    <Grid container item xs={12} key={key} spacing={1}
                                                        ...
                                                        <Grid item xs={2}>
                                                            <HeroTextField id={"weight" + key} label="Waga:"
                                                                           value={eq.weight}
                                                                           inputProps={{
                                                                               min: 0,
                                                                               style: {textAlign: "center"}
                                                        ...
                                                        <Grid item xs={2}>
                                                            <ArmoryButton variant="contained" color="primary"
                                                                          onClick={() => handleAddToCart(eq.name, eq.type, eq.price,eq.weight)}>Kup</ArmoryButton>
            ...
            <DialogActions>
                ...
                <SaveButton onClick={saveEq} color="primary">
                    Kup
                </SaveButton>
            </DialogActions>
